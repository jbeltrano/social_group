...
def obtener_calificaciones_por_receta(receta_id):
    return Calificacion.objects.filter(receta__id=receta_id).order_by('-puntaje')

def obtener_calificacion(receta_id, usuario_correo):
    return Calificacion.objects.filter(receta_id=receta_id, usuario_id=usuario_correo).first()
...
def es_calificacion(receta_id, usuario_correo):
    return Calificacion.objects.filter(receta_id=receta_id, usuario_id=usuario_correo).exists()
